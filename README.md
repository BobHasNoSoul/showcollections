...
THIS VERSION BELLOW WILL GIVE YOU IT VIA A COLLECTION DISPLAY NOT A ALL ITEMS IN THE COLLECTION DISPLAY.

<img width="1920" height="2816" alt="Screenshot 2025-09-05 at 20-34-10 BlueBoxofDOOM" src="https://github.com/user-attachments/assets/9c596004-d74d-4e95-8396-a666741727bf" />

same as the original for where to put it and what file to edit simply add this under the first line and save it and reload your cache.
...
(function() {
    ...
    const createCollectionSection = () => {
        ...
        let header = document.createElement("h2");
        header.innerText = "Collections containing this item";
        collectionContainer.appendChild(header);
    ...
    const fetchCollectionItems = async (itemId) => {
        ...
        for (const boxset of boxsets) {
            ...
            if (isInCollection) {
                console.debug(`Item ${itemId} found in boxset ${boxset.Name}`);
                matchingBoxsets.push(boxset);
            }
        }

        if (matchingBoxsets.length > 0) {
            const header = document.querySelector("#collectionItemsContainer h2");
            header.innerText = "Collections containing this item";

            matchingBoxsets.forEach(boxset => {
                const card = document.createElement("div");
                card.style.width = "150px";
                card.style.textAlign = "center";

                const link = document.createElement("a");
                link.href = `#/details?id=${boxset.Id}&serverId=${new URLSearchParams(window.location.hash.slice(1)).get('serverId')}`;
                link.style.textDecoration = "none";
                link.style.color = "inherit";

                const img = document.createElement("img");
                img.src = `${domain}/Items/${boxset.Id}/Images/Primary?tag=${boxset.ImageTags.Primary || ''}&quality=90`;
                img.style.width = "100%";
                img.style.height = "auto";
                img.style.borderRadius = "5px";
                img.style.objectFit = "contain";
            ...
            console.debug(`Added ${matchingBoxsets.length} collections to section`);
        } else {
            console.debug("Item not found in any collection");
            itemsContainer.innerText = "This item is not in any collection.";
        }
